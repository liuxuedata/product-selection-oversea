...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const docType = Array.isArray(fields.docType) ? fields.docType[0] : (fields.docType as string | undefined);
    ...
    // 3) 立即返回，让后续处理在后台进行
    res.status(200).json({ fileId: fileRow.id });

    // 4) 异步处理文件内容
    processFile(fileRow.id, file, docType).catch((e) => {
...
async function processFile(fileId: number, file: FormidableFile, docType?: string) {
  ...
  console.log('upload stats', { fileId, inserted, skipped, invalid });
}
